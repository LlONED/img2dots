...
function resolutionAdaptation(width, height, maxWidth) {
...
function getDotsHTML(MonochromeImage = [], { type = 6, isFill = false }) {
    ...
    for (let y = 0; y < MonochromeImage.length; y += 3) {
        ...
        for (let x = 0; x < MonochromeImage[y].length; x += 2) {
            let binaryKey = ''
                + (MonochromeImage[y][x] || 0).toString()
                + (MonochromeImage[y][x + 1] || 0).toString()
                + (MonochromeImage[y + 1] !== undefined ? (MonochromeImage[y + 1][x] || 0) : 0).toString()
                + (MonochromeImage[y + 1] !== undefined ? (MonochromeImage[y + 1][x + 1] || 0) : 0).toString()
                + (MonochromeImage[y + 2] !== undefined ? (MonochromeImage[y + 2][x] || 0) : 0).toString()
                + (MonochromeImage[y + 2] !== undefined ? (MonochromeImage[y + 2][x + 1] || 0) : 0).toString();
...
function getMonochromeImage(imgData = [], width = 0, { isInvert = false, sensitivity = 383 }) {
    ...
    for (i = 0; i < imgData.data.length; i += 4) {
        ...
        let colour = isInvert ? 1 : 0;
        if (count > sensitivity) colour = isInvert ? 0 : 1;

        if (i / 4 % width === 0) {
            MonochromeImage.push([colour]);
            if (i > 0) j++;
            continue;
        }

        MonochromeImage[j].push(colour);
    }
